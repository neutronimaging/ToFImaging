...
def EdgeProfileFitting(signal,spectrum,spectrum_range=[],est_pos=0,est_a=0,est_b=0,est_sigma_0=1,est_sigma_1=1,bool_SG=False,SG_w=5,SG_n=1,bool_print=False): 
    # l = wavelenght/tof/bin index
    # d = fitted edge position
    # a = rising parameter
    # b = decaying parameter
    # sigma_0 = wavelength resolution parameter
    # sigma_1 = micro-strain parameter

    sqrt2 = np.sqrt(2)
    def w(l,d,sigma_0,sigma_1):
        return (l-2*d)/(sqrt2*np.sqrt(sigma_0^2+sigma_1^2))
        
    def u(l,d,sigma_0,sigma_1,a):
        return (a*((sigma_0^2+sigma_1^2))^2 + 2*(l-2*d))*a/2

    def v(l,d,sigma_0,sigma_1,b):
        return (b*(sigma_0^2+sigma_1^2)^2 + 2*(l-2*d))*b/2

    def y(l,d,sigma_0,sigma_1,a):
        return (a*(sigma_0^2+sigma_1^2)^2+l-2*d)/(sqrt2*np.sqrt(sigma_0^2+sigma_1^2))
        
    def z(l,d,sigma_0,sigma_1,b):
        return (b*(sigma_0^2+sigma_1^2)^2+l-2*d)/(sqrt2*np.sqrt(sigma_0^2+sigma_1^2))

    def edge_profile(l,d,sigma_0,sigma_1,a,b):
        return special.erfc(w(l,d,sigma_0,sigma_1))/2 - (b*np.exp(u(l,d,sigma_0,sigma_1,a))*special.erfc(y(l,d,sigma_0,sigma_1,a)) - a*np.exp(v(l,d,sigma_0,sigma_1,b))*special.erfc(z(l,d,sigma_0,sigma_1,b)))/(2*(a+b))
    ...
    if(bool_SG):
        signal = SG_filter(signal,SG_w,SG_n)
    ...
    gmodel = Model(edge_profile)
    params = gmodel.make_params(d=t0_f, a = est_a, b = est_b, sigma_0 = est_sigma_0, sigma_1 = est_sigma_1)
    method ='least_squares'
    result1 = gmodel.fit(signal, params, t=t, method=method, nan_policy='propagate')
    t0_f = result1.best_values.get('d')
    sigma_0_f = result1.best_values.get('sigma_0')
    sigma_1_f = result1.best_values.get('sigma_1')
    a_f = result1.best_values.get('a')
    b_f = result1.best_values.get('b')
    ...
    if (bool_print):
        ...
        plt.plot(t0_f,result1.best_fit[index_t0],'ok')
        # plt.plot(t[pos_extrema[0]],result7.best_fit[pos_extrema[0]],'ok')
        # plt.plot(t[pos_extrema[1]],result7.best_fit[pos_extrema[1]],'ok')
        plt.title('Edge fitting with intermediate steps and estimated edge position')
        plt.show()
    ...
    return {'t0':t0_f, 'sigma_0':sigma_0_f, 'sigma_1':sigma_1_f, 'a':a_f, 'b':b_f}
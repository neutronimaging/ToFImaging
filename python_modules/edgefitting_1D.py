...
def AdvancedBraggEdgeFitting(signal,
    ...
    t0_f = t[est_pos_idx]  # this is the actual estimated first position in TOF [s]
    est_w=est_w/2
    ...
    if (bool_print):
        ...
        plt.plot(t,
                 BraggEdgeExponential(t, t0_f, alpha_f, sigma_f, a1_f, a2_f,
                                      a5_f, a6_f),
                 label='Fit')
        plt.title('1st iteration: a1, a6'), plt.xlabel(
    ...
    if (bool_print):
        ...
        plt.plot(t,
                 BraggEdgeExponential(t, t0_f, alpha_f, sigma_f, a1_f, a2_f,
                                      a5_f, a6_f),
                 label='Fit')
        plt.title('2nd iteration: a2, a5'), plt.xlabel(
            'Wavelenght [Å]'), plt.ylabel('Transmission I/I$_{0}$')
        plt.show(), plt.close()
    ...
    if (bool_print):
        ...
        plt.plot(t,
                 BraggEdgeExponential(t, t0_f, alpha_f, sigma_f, a1_f, a2_f,
                                      a5_f, a6_f),
                 label='Fit')
        plt.title('3nd iteration: t0'), plt.xlabel(
            'Wavelenght [Å]'), plt.ylabel('Transmission I/I$_{0}$')
        plt.show(), plt.close()
    ...
    if (bool_print):
        ...
        plt.plot(t,
                 BraggEdgeExponential(t, t0_f, alpha_f, sigma_f, a1_f, a2_f,
                                      a5_f, a6_f),
                 label='Fit')
        plt.title('4nd iteration: t0, sigma, alpha'), plt.xlabel(
            'Wavelenght [Å]'), plt.ylabel('Transmission I/I$_{0}$')
        plt.show(), plt.close()
    ...
    if (bool_print):
        ...
        plt.plot(t,
                 BraggEdgeExponential(t, t0_f, alpha_f, sigma_f, a1_f, a2_f,
                                      a5_f, a6_f),
                 label='Fit')
        plt.title('5nd iteration: a1,a2,a5,a6'), plt.xlabel(
            'Wavelenght [Å]'), plt.ylabel('Transmission I/I$_{0}$')
        plt.show(), plt.close()
    ...
    if (bool_print):
        ...
        plt.plot(t,
                 BraggEdgeExponential(t, t0_f, alpha_f, sigma_f, a1_f, a2_f,
                                      a5_f, a6_f),
                 label='Fit')
        plt.title('6nd iteration: t0, sigma, alpha'), plt.xlabel(
            'Wavelenght [Å]'), plt.ylabel('Transmission I/I$_{0}$')
        plt.show(), plt.close()
...
def ProfileBraggEdgeFitting(signal,
                            spectrum,
                            spectrum_range=[],
                            est_pos=0,
                            est_sigma=1,
    ...
    #----------------- FITTING FUNCTIONS---------------#
    def w(t, t0, sigma):
        return (t - t0) / (np.sqrt(2) * sigma)

    def u(t, t0, alpha, sigma):
        return alpha / 2 * (alpha * sigma**2 + 2 * (t - t0))

    def v(t, t0, beta, sigma):
        return beta / 2 * (beta * sigma**2 - 2 * (t - t0))

    def y(t, t0, alpha, sigma):
        return (alpha * sigma**2 + (t - t0)) / (np.sqrt(2) * sigma)

    def z(t, t0, beta, sigma):
        return (beta * sigma**2 - (t - t0)) / (np.sqrt(2) * sigma)

    def PeakProfile(t, t0, beta, alpha, sigma, off, slope):
        return (off + slope * t) + (0.5 * special.erfc(w(t, t0, sigma)) -
                                    (beta * np.exp(u(t, t0, alpha, sigma)) *
                                     special.erfc(y(t, t0, alpha, sigma)) -
                                     alpha * np.exp(v(t, t0, beta, sigma)) *
                                     special.erfc(z(t, t0, beta, sigma))) /
                                    (2 * (alpha + beta)))
    ...
    sigma_f = result.best_values.get('sigma')
    alpha_f = result.best_values.get('alpha')
    beta_f = result.best_values.get('beta')
    off_f = result.best_values.get('off')
    slope_f = result.best_values.get('slope')
    ...
    if (bool_print):
        ...
        plt.plot(t,
                 PeakProfile(t, t0_f, est_beta, est_alpha, est_sigma, est_off,
                             est_slope),
                 label='Initial guess')
        plt.plot(t,
                 PeakProfile(t, t0_f, beta_f, alpha_f, sigma_f, off_f,
                             slope_f),
                 label='Fit')
        plt.title('Final iteration: all'), plt.xlabel(
            'Wavelenght [Å]'), plt.ylabel('Transmission I/I$_{0}$')

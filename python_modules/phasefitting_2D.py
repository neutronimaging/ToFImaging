...
def phase_ratio_linearcomb_2D(lac_tof,
                              spectrum,
                              phase1lac,
                              phase2lac,
                              spectrum_phase,
    ...
    if (any(debug_idx)):  #testing on a single pixel
        ...
        phasefitting_1D.phase_ratio_linearcomb(lac,
                                               lambd,
                                               phase1lac,
                                               phase2lac,
                                               spectrum_phase,
    ...
    for i in tqdm(range(0, np.shape(mymask)[0])):
        for j in range(0, np.shape(mymask)[1]):
            if (mymask[i, j]):
                ...
                try:
                    phi_fit = phasefitting_1D.phase_ratio_linearcomb(
                        lac,
                        lambd,
                        phase1lac,
                        phase2lac,
...
def phase_ratio_linearcomb_three_2D(lac_tof,
                                    spectrum,
                                    phase1lac,
                                    phase2lac,
                                    phase3lac,
    ...
    if (any(debug_idx)):  #testing on a single pixel
        ...
        phasefitting_1D.phase_ratio_linearcomb_three(lac,
                                                     lambd,
                                                     phase1lac,
                                                     phase2lac,
                                                     phase3lac,
    ...
    for i in tqdm(range(0, np.shape(mymask)[0])):
        for j in range(0, np.shape(mymask)[1]):
            if (mymask[i, j]):
                ...
                try:
                    phi_fit = phasefitting_1D.phase_ratio_linearcomb_three(
                        lac,
                        lambd,
                        phase1lac,
                        phase2lac,
...
def WavelengthSelectiveRatio2D(lac_tof,
    ...
    if (any(debug_idx)):  #testing on a single pixel
        ...
        phasefitting_1D.WavelengthSelectiveRatio(lac,
                                                 lambd,
                                                 l1,
                                                 l2,
                                                 l1_w,
    ...
    for i in tqdm(range(0, np.shape(mymask)[0])):
        for j in range(0, np.shape(mymask)[1]):
            if (mymask[i, j]):
                ...
                try:
                    phi_fit = phasefitting_1D.WavelengthSelectiveRatio(
                        lac,
                        lambd,
                        l1,
                        l2,
